...
import { getAboutSettings, saveAboutSettings } from "@/lib/actions";
...
export default function AdminAboutSettingsPage() {
  ...
  useEffect(() => {
    async function fetchSettings() {
        setIsLoading(true);
        const currentSettings = await getAboutSettings();
        if (currentSettings) {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
        ...
        const result = await saveAboutSettings({ 
            ...settings,
        ...
        if (result.success) {
            toast({
                ...
                description: "Pengaturan halaman tentang Anda telah diperbarui.",
            });
      ...
      <div className="grid grid-cols-1 gap-8">
          <Card>
              <CardHeader>
                  ...
                  <CardDescription>Kelola konten untuk halaman "Tentang" Anda.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                    <div className="space-y-2">
                   ...
                   <div className="space-y-2">
                       ...
                       <Input id="headline" name="headline" value={settings.headline || ""} onChange={handleInputChange} placeholder="Contoh: Ningrat Stories lahir dari..." disabled={isSaving} />
                   </div>
                    <div className="space-y-2">
                       <Label htmlFor="paragraph">Paragraf</Label>
                       <Textarea id="paragraph" name="paragraph" value={settings.paragraph || ""} onChange={handleInputChange} placeholder="Kami didorong oleh hasrat untuk..." className="min-h-[150px]" disabled={isSaving} />
                   </div>
              </CardContent>
          </Card>
