...
import { Upload, Loader2, Trash2, ArrowUp, ArrowDown, Library, Link as LinkIcon, Video } from "lucide-react";
...
export default function EditBlogPostPage() {
  ...
  const [selectionTarget, setSelectionTarget] = useState<{ blockId: string, maxFiles: number } | 'hero' | null>(null);
   ...
   useEffect(() => {
    setPageTitle('Ubah Postingan Blog');
  }, [setPageTitle]);


  const form = useForm<z.infer<typeof blogPostSchema>>({
    resolver: zodResolver(blogPostSchema),
    defaultValues: { title: "", author: "", excerpt: "" },
  });
  ...
  const fetchPostData = async (id: string) => {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            form.reset({ title: data.title, author: data.author, excerpt: data.excerpt || "" });
            setHeroImageUrl(data.heroImageUrl || null);
            setContentBlocks(data.contentBlocks || []);
        } else {
            toast({ title: "Tidak Ditemukan", description: "Postingan blog tidak dapat ditemukan.", variant: "destructive" });
  ...
  const onSubmit = async (values: z.infer<typeof blogPostSchema>) => {
    ...
    try {
      ...
      toast({ title: "Finalisasi...", description: "Menyimpan postingan blog yang diperbarui." });
      ...
      if (result.success) {
        toast({ title: "Postingan Blog Diperbarui!", description: "Postingan blog Anda telah berhasil diperbarui." });
        router.push("/admin/blog");
      } else {
        throw new Error(result.message || "Gagal memperbarui postingan blog.");
      }

    } catch (error: any) {
      console.error("Gagal memperbarui postingan blog:", error);
